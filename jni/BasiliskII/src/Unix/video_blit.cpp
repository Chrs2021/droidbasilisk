...
static void Blit_Copy_Raw(uint8 * dest, const uint8 * source, uint32 length)
{
	...
	//slow & dirty hack to make it work

    const uint8 * srcp = source;
    uint8 * dstp = dest;
    ...
    //faster version?
    /*
    const uint32 *srcp = (uint32*) source;
...
#define FB_BLIT_2(dst, src) \
    (dst = 0xff000000)
    //(dst = (((src) >> 24) & 0xff) | (((src) >> 8) & 0xff00) | (((src) & 0xff00) << 8) | (0xff << 24))
    //(dst = (((src) >> 16) & 0xff00) | (((src) << 8) & 0xff00) | (((src) & 0xff00) << 8) | ((src) & 0xff) )
...
#define FB_BLIT_2(dst, src) \
    (dst = 0xff000000)
    //(dst = (((src) >> 16) & 0xff) | ((src) & 0xff00) | (0xff << 16))
...
#define FB_BLIT_2(dst, src) \
    (dst = 0xff000000)
    //(dst = (((src) >> 16) & 0xff) | ((src) & 0xff0000) | (0xff << 16))
...
static void Blit_Expand_1_To_32(uint8 * dest, const uint8 * p, uint32 length)
{
  ...
  /*
		...
		*/
		test = -(c >> 7);
		if (test > 0) { dstp[0]=0; dstp[1]=0; dstp[2]=0; dstp[3]=255; dstp+=4;}
    if (test ==0) { dstp[0]=255; dstp[1]=255; dstp[2]=255; dstp[3]=0; dstp+=4;}
		test = -((c >> 6) & 1);
		if (test > 0) { dstp[0]=0; dstp[1]=0; dstp[2]=0; dstp[3]=255; dstp+=4;}
    if (test ==0) { dstp[0]=255; dstp[1]=255; dstp[2]=255; dstp[3]=0; dstp+=4;}
		test = -((c >> 5) & 1);
...
#else
#warning "little endian blitters"
	{  1, 0x000000, 0x000000, 0x000000, Blit_Copy_Raw	, Blit_Copy_Raw		},	// NT
	{  8, 0x000000, 0x000000, 0x000000, Blit_Copy_Raw	, Blit_Copy_Raw		},	// OK (NBO)
...
{
    ALOG_DEBUG("Screen blitter init...");
#if USE_SDL_VIDEO
...
//#warning "blitting using real addressing"
...
//	} else {
		...
		case 24:
		case 32:
            ALOG_DEBUG("32bits blitters");
			switch (mac_depth) {
			    ALOG_DEBUG("Mac dept: %d", mac_depth);
        ...
        if (!blitter_found) {
            //try raw UAE blit (32bits)
            ALOG_DEBUG("32->32, raw blitter to be used");
            Screen_blit = Blit_Copy_Raw;
        }
/*
		...
		for (int i = 0; !blitter_found && (i < blitters_count); i++) {
			...
			{
				blitter_found = true;
				ALOG_DEBUG("Blitter found:%d, byte order:%d", i, native_byte_order);
