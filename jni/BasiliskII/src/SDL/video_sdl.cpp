...
#else
const char KEYCODE_FILE_NAME[] = "/sdcard/keycodes";
#endif
	...
	: driver_base(m), mouse_grabbed(false)
{
	...
	// commented on Android as it closes the app :(
    ...
    //rgb -> rbg
    //visualFormat.Rmask = f->Bmask;
    //visualFormat.Gmask = f->Gmask;
	//visualFormat.Bmask = f->Rmask;
	Screen_blitter_init(visualFormat, true, mac_depth_of_video_depth(VIDEO_MODE_DEPTH));
	ALOG_DEBUG("blitting settup: %d, %d", depth, mac_depth_of_video_depth(VIDEO_MODE_DEPTH));
...
driver_fullscreen::driver_fullscreen(SDL_monitor_desc &m)
	: driver_base(m)
{
    ALOG_DEBUG("in driver_fullscreen");
	int width = VIDEO_MODE_X, height = VIDEO_MODE_Y;
	int aligned_width = (width + 15) & ~15;
	int aligned_height = (height + 15) & ~15;
...
bool SDL_monitor_desc::video_open(void)
{
	...
	case DISPLAY_WINDOW:
		...
		ALOG_DEBUG("driver_window: bytes per row:%d", drv->s->pitch);
		break;
	case DISPLAY_SCREEN:
...
void SDL_monitor_desc::set_palette(uint8 *pal, int num_in)
{
	...
	for (int i=0; i<num_out; i++) {
        ...
        p->b = pal[c*3 + 0] * 0x0101;
		p->g = pal[c*3 + 1] * 0x0101;
//		p->b = pal[c*3 + 2] * 0x0101;
        p->r = pal[c*3 + 2] * 0x0101;
		p++;
	}
	...
	if (!IsDirectMode(mode)) {
		for (int i=0; i<256; i++) {
			...
			ExpandMap[i] = SDL_MapRGB(drv->s->format, pal[c*3+2], pal[c*3+1], pal[c*3+0]);
		}
...
static void update_display_static(driver_base *drv)
{
    //ALOG_DEBUG("update display static");
	// Incremental update code
	int wide = 0, high = 0, x1, x2, y1, y2, i, j;
	const VIDEO_MODE &mode = drv->mode;
	const int dst_bytes_per_pixel = 4;
	...
	if (high) {
		if ((int)VIDEO_MODE_DEPTH < VIDEO_DEPTH_8BIT) {
			...
			if (high && wide) {
				...
				int di = y1 * dst_bytes_per_row + dst_bytes_per_pixel*x1;
				for (j = y1; j <= y2; j++) {
			...
			if (high && wide) {
				...
				for (j=y1; j<=y2; j++) {
					...
					int dst_i = j * dst_bytes_per_row + x1 * dst_bytes_per_pixel;
...
{
    //ALOG_DEBUG("update display bbox");
	const VIDEO_MODE &mode = drv->mode;
	...
	for (y = 0; y < VIDEO_MODE_Y; y += N_PIXELS) {
		...
		for (x = 0; x < VIDEO_MODE_X; x += N_PIXELS) {
			...
			for (int j = y; j < (y + h); j++) {
				...
				const int dst_xb = x * dst_bytes_per_pixel;
				if (memcmp(&the_buffer[yb + xb], &the_buffer_copy[yb + xb], xs) != 0) {
					memcpy(&the_buffer_copy[yb + xb], &the_buffer[yb + xb], xs);
					Screen_blit((uint8 *)drv->s->pixels + dst_yb + dst_xb, the_buffer + yb + xb, xs);
					dirty = true;
				}
			}
			if (dirty) {
...
{
    //ALOG_DEBUG("video refresh dga vosf");
	// Quit DGA mode if requested
	possibly_quit_dga_mode();
...
{
   // ALOG_DEBUG("video refresh window vosf");
...
static void video_refresh_window_static(void)
{
    //ALOG_DEBUG("video refresh static");
	// Ungrab mouse if requested
	possibly_ungrab_mouse();
...
static void VideoRefreshInit(void)
{
	...
	if (display_type == DISPLAY_SCREEN) {
		...
		if (use_vosf)
			...
			//ALOG_DEBUG("video refresh init window static");
            }
	}
}
